...
export const eventCard = (e) => { const p = dateParts(e.start); const multi = e.end !== e.start; return `<article class="card event ev-${e.type}" tabindex="0" role="button" data-event="${esc(JSON.stringify({ title: e.title, start: e.start, end: e.end, time: e.time, place: e.place, audience: e.audience, description: e.description, type: e.type }))}"><div class="date"><b>${p.d}</b><span>${p.m}</span></div><div><h3>${esc(e.title)}</h3><p>${multi ? `${fmtShort(e.start)} – ${fmtShort(e.end)}` : e.time || 'celý den'}${e.place ? ' · ' + esc(e.place) : ''}</p><span class="chip chip-mute">${e.audience}</span></div></article>`; };
export const EVENT_RENDER_JS = `window.renderEvent=function(e){var m=['led','úno','bře','dub','kvě','čvn','čvc','srp','zář','říj','lis','pro'];var d=e.start.split('-');var s=function(x){var p=x.split('-');return +p[2]+'. '+ +p[1]+'.'};var attr=JSON.stringify(e).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;');return '<article class="card event ev-'+e.type+'" tabindex="0" role="button" data-event="'+attr+'"><div class="date"><b>'+ +d[2]+'</b><span>'+m[+d[1]-1]+'</span></div><div><h3>'+e.title+'</h3><p>'+(e.end!==e.start?s(e.start)+' – '+s(e.end):(e.time||'celý den'))+(e.place?' · '+e.place:'')+'</p><span class="chip chip-mute">'+e.audience+'</span></div></article>'};`;
